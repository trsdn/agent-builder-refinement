...
flowchart TD
    ...
    %% Main blocks
    P[Purpose] --> G[Guidelines]
    G --> S[Skills]

    %% Guideline sub-blocks
    G --> GD[General directions]
    G --> T[Tone]
    G --> R[Restrictions]
    G --> O[...]

    %% Skills
    S --> S1[Skill 1<br/>Description<br/>Step-by-step workflow<br/>Sample output]
    S --> S2[Skill 2<br/>Description<br/>Sample output]
    S --> S3[Skill 3<br/>Description<br/>Step-by-step workflow]
    S --> SN[Skill n<br/>...]

    %% Styling
    classDef purpose fill:#1fa34a,stroke:#0f5e2a,color:#fff;
    classDef guidelines fill:#0094ff,stroke:#005c99,color:#fff;
    classDef skills fill:#b4f5c2,stroke:#6bbf8a,color:#000;

    class P purpose;
    class G,GD,T,R,O guidelines;
    class S,S1,S2,S3,SN skills;
```
